...
fastify.post('/api/admin/auth', async (request, reply) => {
    try {
        ...
        const result = await pool.query(`SELECT u.id_user, u.username, u.role, 
             ...
             WHERE u.id_user = $1 AND u.id_org = $2 AND u.role = 'admin'`, [id_user, id_org]);
...
fastify.post('/api/admin/settings', async (request, reply) => {
    try {
        ...
        await pool.query(`INSERT INTO admin_settings (setting_key, setting_value, updated_by, updated_at) 
             VALUES ($1, $2, $3, CURRENT_TIMESTAMP)
             ON CONFLICT (setting_key) 
             DO UPDATE SET setting_value = $2, updated_by = $3, updated_at = CURRENT_TIMESTAMP`, [key, value, admin_user]);
        return { success: true };
    }
    catch (err) {
...
fastify.post('/api/admin/user/permissions', async (request, reply) => {
    try {
        ...
        await pool.query(`INSERT INTO admin_users (id_user, can_manage_themes, can_manage_users, can_manage_roles, can_view_logs)
             VALUES ($1, $2, $3, $4, $5)
             ON CONFLICT (id_user)
             DO UPDATE SET 
                can_manage_themes = $2,
...
fastify.post('/api/admin/sql', async (request, reply) => {
    try {
        ...
        // Проверка прав
        const adminCheck = await pool.query('SELECT can_manage_users FROM admin_users WHERE id_user = $1', [admin_user]);
...
fastify.get('/api/admin/logs', async (request, reply) => {
    try {
        ...
        const adminCheck = await pool.query('SELECT can_view_logs FROM admin_users WHERE id_user = $1', [admin_user]);
...
fastify.post('/api/admin/upload-logo', async (request, reply) => {
    try {
        ...
        await pool.query(`INSERT INTO admin_settings (setting_key, setting_value, setting_type) 
             VALUES ('logo_url', $1, 'url')
             ON CONFLICT (setting_key) 
             DO UPDATE SET setting_value = $1`, [logoUrl]);
        return { url: logoUrl };
    }
    catch (err) {
...
fastify.get('/api/admin/links', async (request, reply) => {
    try {
        const result = await pool.query("SELECT setting_key, setting_value FROM admin_settings WHERE setting_key LIKE 'link_%'");
        const links = {};
        result.rows.forEach(row => {
...
fastify.post('/api/admin/links', async (request, reply) => {
    try {
        ...
        for (const [key, value] of Object.entries(links)) {
            await pool.query(`INSERT INTO admin_settings (setting_key, setting_value, setting_type, updated_by) 
                 VALUES ($1, $2, 'url', $3)
                 ON CONFLICT (setting_key) 
                 DO UPDATE SET setting_value = $2, updated_by = $3`, [key, value, admin_user]);
        }
        return { success: true };
    }
    catch (err) {
...
fastify.get('/api/admin/theme', async (request, reply) => {
    try {
        const result = await pool.query("SELECT setting_key, setting_value FROM admin_settings WHERE setting_type IN ('color', 'css')");
        const theme = {};
        result.rows.forEach(row => {
...
fastify.post('/api/admin/settings', async (request, reply) => {
    try {
        const { key, value, admin_user } = request.body;
        // Проверка прав
        const adminCheck = await pool.query('SELECT can_manage_themes FROM admin_users WHERE id_user = $1', [admin_user]);
...
fastify.post('/api/admin/user/permissions', async (request, reply) => {
    try {
        ...
        const adminCheck = await pool.query('SELECT can_manage_users, can_manage_roles FROM admin_users WHERE id_user = $1', [admin_user]);
        ...
        await pool.query(`INSERT INTO admin_users (id_user, can_manage_themes, can_manage_users, can_manage_roles, can_view_logs)
             ...
             DO UPDATE SET 
                ...
                can_manage_users = $3,
                can_manage_roles = $4,
                can_view_logs = $5`, [target_user, permissions.can_manage_themes, permissions.can_manage_users,
...
fastify.get('/api/admin/theme', async (request, reply) => {
    try {
        const settings = await pool.query('SELECT setting_key, setting_value FROM admin_settings WHERE setting_type IN ($1, $2, $3)', ['color', 'css', 'url']);
        const theme = {};
        ...
        io.on('connection', async (socket) => {
            ...
            try {
                ...
                await pool.query(`INSERT INTO users (id_user, id_org, username, role) 
                    VALUES ($1, $2, $3, $4) ON CONFLICT (id_user) DO UPDATE SET role = $4`, [id_user, id_org, username, role]);
                await pool.query(`INSERT INTO rooms (id_room, id_org, type, name, created_by) 
                    VALUES (1, $1, 'general', 'Общий чат', $2) ON CONFLICT (id_room) DO NOTHING`, [id_org, id_user]);
            ...
            socket.on('get_users_list', async () => {
                try {
                    const usersResult = await pool.query('SELECT id_user, username, role FROM users WHERE id_org = $1 AND id_user != $2', [id_org, id_user]);
            ...
            socket.on('add_user_to_group', async (data) => {
                ...
                try {
                    ...
                    const room = await pool.query('SELECT type FROM rooms WHERE id_room = $1 AND id_org = $2', [data.room_id, id_org]);
                    if (room.rows[0]?.type !== 'group') {
                        socket.emit('error', { message: 'Это не групповая комната' });
                        return;
                    }
                    await pool.query('INSERT INTO room_participants (id_room, id_user) VALUES ($1, $2) ON CONFLICT DO NOTHING', [data.room_id, data.user_id]);
            ...
            socket.on('remove_user_from_group', async (data) => {
                ...
                try {
                    ...
                    await pool.query('DELETE FROM room_participants WHERE id_room = $1 AND id_user = $2', [data.room_id, data.user_id]);
                    io.to(`org_${id_org}`).emit('refresh_rooms_trigger');
            ...
            socket.on('create_private_chat', async (data) => {
                try {
                    ...
                    const newRoom = await pool.query('INSERT INTO rooms (id_org, type, name, created_by) VALUES ($1, $2, $3, $4) RETURNING id_room', [id_org, 'private', roomName, id_user]);
                    const newRoomId = newRoom.rows[0].id_room;
