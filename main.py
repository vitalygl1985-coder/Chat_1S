...
def init_db():
    ...
    try:
        ...
        # Создаем админский кабинет
        cur.execute("SELECT 1 FROM rooms WHERE name = 'АДМИНИСТРАТОРЫ' LIMIT 1")
        if not cur.fetchone():
            cur.execute("""
                INSERT INTO rooms (id_org, type, name, created_by)
                VALUES ('00000000-0000-0000-0000-000000000001'::uuid, 'admin_group', 'АДМИНИСТРАТОРЫ', 'system')
            """)
...
async def auth_user(data: AuthRequest):
    ...
    try:
        ...
        # Добавляем админов в админский кабинет
        if data.role == 'admin':
            cur.execute("SELECT id_room FROM rooms WHERE name = 'АДМИНИСТРАТОРЫ' LIMIT 1")
            admin_room = cur.fetchone()
            if admin_room:
                cur.execute("INSERT INTO room_participants (id_room, id_user) VALUES (%s, %s) ON CONFLICT DO NOTHING",
                           (admin_room[0], data.id_user))
...
async def admin_auth(data: AuthRequest):
    validated_org = clean_uuid(data.id_org)
    conn = get_db_connection()
    cur = conn.cursor(cursor_factory=RealDictCursor)
    try:
